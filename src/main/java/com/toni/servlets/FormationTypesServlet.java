...
/**
 * Servlet implementation class FormationTypesServlet
 */
@WebServlet("/FormationTypesServlet")
public class FormationTypesServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * Default constructor. 
     */
    public FormationTypesServlet() {
					...
					if("A".equals(typologyCode)) {
						...
						try {
							if("delete".equals(action)) {
								formationTypesService.deleteFormationType(request);
							}
						...
						request.getServletContext().getRequestDispatcher("/WEB-INF/formationTypes.jsp").forward(request, response);
					} else {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(authenticationService.isAuthenticated(request)) {
			...
			if("A".equals(typologyCode)) {
				FormationTypesService formationTypesService = FormationTypesService.getInstance();
				try {
					request.setAttribute("formationTypes", formationTypesService.findFormationType(request));
				} catch (ClassNotFoundException | SQLException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
